...
class ControllerPaymentCarpayulatam extends Controller {
	protected function index() {
		$this->language->load('payment/carpayulatam');
		...
		$merchantId = $this->config->get('carpayulatam_merchantId');			
		$apiKey = $this->config->get('carpayulatam_apiKey');			
		$apiLogin = $this->config->get('carpayulatam_apiLogin');			
		$publicKey = $this->config->get('carpayulatam_publicKey');			
		$accountId = $this->config->get('carpayulatam_accountId');			
		$moneda = $currency;
                $referenceCode = $order_info['order_id'];
			
		
	$this->data['merchantId'] = $merchantId;
	$this->data['referenceCode'] = $referenceCode;
	$this->data['test'] = $this->config->get('carpayulatam_prueba');
        $this->data['buyerEmail'] = isset($this->session->data['guest']['email'])? $this->session->data['guest']['email']:$this->customer->getEmail();
	...
	$this->data['confirmationUrl']=HTTPS_SERVER.'/pol/confirmacion.php';
	$this->data['description'] = "Pedido # ".$order_info['order_id']." en ".$this->config->get('config_name');
		$baseDevolucionIva = 0;
                $desc_products = array ();
	...
	$this->data['taxReturnBase']=$baseDevolucionIva;
	$this->data['tax'] = $tax_total;	
	$this->data['currency'] = $currency;
	$this->data['amount'] =  $valor;
        ...
        $signature = $apiKey.'~'.$merchantId.'~'.$referenceCode.'~'.$valor.'~'.$currency;
        $signature = md5($signature);
        $this->data['signature'] =  $signature;        
		$this->id = 'payment';

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/carpayulatam.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/carpayulatam.tpl';
		} else {
			$this->template = 'default/template/payment/carpayulatam.tpl';
		}
                
                
		$this->model_checkout_order->confirm($order_info['order_id'],$this->config->get('carpayulatam_order_status_id'));
		$this->render();		
	}
